  ...
  methods: {
    ...
    editItem(item) {
      ...
      for (let key in this.properties) {
        if (this.properties[key].id == this.currentId) {
          this.editedProperties = Object.assign({}, this.properties[key]);

          for (let keyAdd in this.editedProperties) {
            if (keyAdd == "id") delete this.editedProperties.id;
          }

          this.inputedProperties = Object.assign({}, this.editedProperties);
        }
      }
    ...
    saveEdit() {
      this.axios
        .put(
          this.apiPath + "/properties/" + this.currentId,
          this.inputedProperties
        )
        .then((response) => {
    ...
    deleteItemConfirm() {
      this.axios
        .delete(this.apiPath + "/properties/" + this.currentId)
        .then((response) => {
          console.log(response);
    ...
    saveAdd() {
      this.axios
        .post(this.apiPath + "/properties", this.addedProperties)
        .then((response) => {
          console.log(response);
              ...
              <v-container>
                <v-row class="mt-10">
                  <v-col
                    ...
                    sm="6"
                    md="6"
                    v-for="(x, index) in editedProperties"
                    :key="`${index}-${x}`"
                  >
                    <v-text-field
                      ref="myInput"
                      :label="fieldLabels[index]"
                      v-model="inputedProperties[index]"
                      filled
                    ></v-text-field>
                  </v-col>
                </v-row>
